...
driveImages.forEach(img => {
  if (img.name.startsWith('cert_')) {
    content = content.replace(`imageUrl: \`\$\{import.meta.env.BASE_URL\}assets/${img.name}\``, `imageUrl: "https://drive.google.com/thumbnail?id=${img.id}&sz=w1000"`);
  } else if (img.name === 'cv_page_1.jpg') {
    content = content.replace(`src={\`\$\{import.meta.env.BASE_URL\}assets/${img.name}\`}`, `src="https://drive.google.com/thumbnail?id=${img.id}&sz=w2000" referrerPolicy="no-referrer"`);
    content = content.replace(`e.currentTarget.src = \`\$\{import.meta.env.BASE_URL\}assets/${img.name}\`;`, `e.currentTarget.src = "https://drive.google.com/uc?export=view&id=${img.id}";`);
    content = content.replace(`url: \`\$\{import.meta.env.BASE_URL\}assets/${img.name}\``, `url: "https://drive.google.com/uc?export=download&id=${img.id}"`);
...
docs.forEach(doc => {
  const rsStr = `href={\`\$\{import.meta.env.BASE_URL\}assets/${doc.name}\`}`;
  let oldContent;
  do {
    oldContent = content;
    content = content.replace(rsStr, `href="${doc.link}"`);
